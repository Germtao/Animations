...
## 什么时候应该使用UIViewPropertyAnimator？

在`iOS`中有如此众多的动画`API`，什么时候最好使用`UIViewPropertyAnimator`？

差异是可中断性。如果要在飞行中“捕获”视图，对其进行擦洗，然后继续或反转动画，则`UIViewPropertyAnimator`是最佳选择。其他一些动画策略是可中断的，例如在滚动视图的`contentOffset`属性旁边设置视图动画，但这些动画仅限于滚动视图的时序曲线。

有时，动画不能被打断是没有意义的。仅当用户可以在动画过程中点击视图时，才使动画可中断。动画应具有足够长的持续时间，以使用户可以足够快地做出反应以点击它，而动画视图应具有较大的点击目标。上面的弹出动画是满足这些要求的完美示例。
